...
%--------------------------------------------------------------------------------------
	...
	\]
		...
		Grâce aux graphiques ci-dessous, nous pouvons nous apercevoir que nos deux graphiques ont une allure en rien comparable, jettant un doute sur le fonctionnement du mode "torque" de notre contrôleur moteur; posant la question du rôle de ce mode de contrôle, si ce derniers n'est pas destiner à être image d'un contrôle en accélération.
		De plus, outre la non adéquation avec le mode de contrôle "Torque", nous nous sommes demandé si le traitement du signal analogique par le contrôleur moteur, pouvait ou non, induire un retard, un décalage temporel entre la consigne mesuré et l'établissement de la valeur de contrôle en sortie du contrôleur.
	...
	Enfin, nous pourrons également mieux tenter de caractériser notre contrôle en accélération, étant donné que nous pourrons comparer des grandeurs physiques de même nature en entrée, comme en sortie du système. Ces données pourrons être analysé au travers d'un diagramme de BAUD.
		...
		Afin de caractériser notre fonction de transfère, nous avons tout dabord commencé par représenter sous forme de graphique notre nouveau système, afin de pouvoir observer les différences entre les deux courbes obtenue.
		On retrouve ci-dessous, trois graphiques à titres d'exemple des mesures effectués; mesures ayant également été prises pour les valeurs de fréquences suivante : 20Hz, 30Hz, 40Hz, 50Hz, 70Hz et 100Hz. 
	...
	Ensuite, nous avons calculé %TODO demander à hervé
