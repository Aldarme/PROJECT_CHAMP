...
\part{Projet "CHAMP", étude et caractérisation}
	...
	le milieu scientifique, représenté par Antoine LETESSIER SELVON, Hervé LEBBOLO et Philippe REPAIN respectivement physicien des hautes énergies et directeur de recherche au CNRS, ingénieurs de recherche et mécanicien chef d'atelier; au travers du LPNHE (laboratoire de physique nucléaire et des hautes énergies.	
	...
	Au travers de cette rencontre, plus que de vouloir proposer un piano de concert connecté, nouvelle génération, le projet prend racine au travers d'une idée simple mais précise, élargir les possibilités de l'instrument en n'en modifiant la mécanique.
	...
	Recruté en tant que stagiaire sur ce projet, pour mon stage de fin d'études en informatique des systèmes embarqués, mon rôle fut de mettre en place, étudier des possibilités et de développer des solutions afin de mener le projet.
	
	Lors de mon arrivée sur le projet, l'équipe du laboratoire avait déjà commencé une étude de cas, s'appuyant sur une base d'électronique analogique. Suite à mon arrivée, nous avons redéfini le cadre du projet, afin de garder les idées et concepts testés et approuvés lors des premiers tests, tout en faisant évoluer le projet en basculant sur une base orientée électronique numérique, système embarqué.
	
	De plus, vis-à-vis de l'aspect technologique, j'ai eu en charge l'étude, la caractérisation et le choix de solution technologique.
	
	Enfin ,j'ai pu être force de proposition quant à l'évolution du projet, étant une preuve de faisabilité, ce base sur un cahier des charges au contour assez large, ce qui induit une plus grande liberté d'esprit, quant aux innovations pouvant être apportées.
	
	
	\chapter{Les travaux menés}
	
	Suite à mon arrivé au sein du LPNHE, j'ai commencer par prendre connaissance de l'architecture globale du projet (figure ci-dessous) et des concepts et idées qui avaient déjà étés pensé et proposé pour le projet, ainsi que des diverses solutions techniques à l'étude concernant des choix de composants.
	
	\hfill\hbox to 0pt{\hss\includegraphics[width=17cm]{CH.png}\hss}\hfill\null\newline
	
	Ainsi, j'ai tout d'abord travailler sur une études de cas entre le bus I2C et le bus SPI afin de déterminer lequel serai le plus adapter à notre projet.
	
	Ensuite, j'ai travailler à mettre en place l'échange de donné entre l'accéléromètre et notre FPGA, puis entre le FGPA et le DAC, en m'appuyant sur le langage VHDL.
	
	Puis, nous avons mis en place un filtrage numérique au sein de notre FPGA. Ce filtrage a pour but de mettre en place un asservissement numérique sur le contrôle des moteur en position.
	...
	Enfin, j'ai réaliser la conception, ainsi que le routage de cartes électroniques, ayant pour but de miniaturiser le plus possible la carte d'évaluation de notre accéléromètre, puis de réaliser une carte sur mesure permettant de connecter plusieurs éléments à de l'électronique embarqué.
	
	\newpage
	
	Au final, notre chaine de mesure, se compose comme suivant:\newline
	
	\hfill\hbox to 0pt{\hss\includegraphics[width=17cm]{CH&MO.png}\hss}\hfill\null\newline
	
	Mais, il est important de prendre en compte que ce module, illustré ci-dessus, n'est que 1/12 ème du projet final.
	En effet, le module présenté ne permet la récupération et le traitement que d'un seul accéléromètre à la fois et donc, que d'une seul touche à la fois.
	
	La première maquette à réaliser comportant 12 touche, il nous faut donc, généraliser notre module afin de récupérer et traiter les informations de 12 accéléromètres afin de les transmettre à 12 moteurs.
	Ce travail étant un peu plus complexe que le module précédemment présenté, il a demandé une refonte d'une partie de la chaine, vis-à-vis de l'échange de données entre le FPGA et le DAC.
